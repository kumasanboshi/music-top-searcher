...
### STEP 6: レビュー・マージ（ダブルチェック体制）

PRのレビューは**Codex**と**Claude Code**の両方で実施し、品質を担保する。

#### 6-1. Codexレビュー

```bash
codex -q "PRの差分をレビューしてください。バグ、セキュリティ問題、設計上の懸念点を指摘してください。"
```

| 観点 | 内容 |
|------|------|
| バグ検出 | ロジックエラー、エッジケース漏れ |
| セキュリティ | XSS、インジェクション等の脆弱性 |
| パフォーマンス | 非効率な実装、N+1問題等 |
| コード品質 | 可読性、命名、重複コード |

#### 6-2. Claude Codeレビュー

Claude Codeでも同様の観点でレビューを実施:

| 観点 | 内容 |
|------|------|
| 設計整合性 | 既存アーキテクチャとの整合性 |
| 型安全性 | TypeScriptの型定義の妥当性 |
| テストカバレッジ | テストケースの十分性 |
| ベストプラクティス | React/TypeScriptのベストプラクティス準拠 |

#### 6-3. レビュー結果の統合

両者のレビュー結果を比較し、以下を確認:
- 両者が指摘した共通の問題 → **必ず対応**
- 一方のみが指摘した問題 → **検討の上で対応を判断**
- 指摘がない場合 → マージ可能
...
| **Codex** | PRレビュー（Claude Codeとのダブルチェック） |
