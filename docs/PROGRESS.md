...
1. **自動検証スクリプト作成** (`scripts/validate-data.mjs`)
   - データ構造の整合性チェック（100曲、連番、必須フィールド）
   - 重複データのチェック
   - アーティスト情報の整合性（ID一貫性）
   - ランキングファイルと曲ファイルの整合性

2. **データ修正スクリプト作成** (`scripts/fix-data-issues.mjs`)
   - 2024年のデータを100件に拡張
   - 欠落している曲ファイルを生成
   - アーティスト名の表記揺れを統一
...
3. **データ修正**
   - 2024年JPOP: 10件 → 100件
   - 2024年western: 10件 → 100件
   - 198件の曲ファイルを生成
   - アーティスト名の表記揺れを修正（EXILE/Exile、アイリーン・キャラ/Irene Cara）
   - 1978年Exile(US)のID修正（a-exile → a-exile-us）
...
|------|--------|--------|
| エラー | 20件 | 0件 |
| 警告 | 22件 | 20件（ft./コラボ、許容） |

### コミット履歴

| ハッシュ | メッセージ |
|---------|-----------|
| `329d7dd8` | fix: 楽曲ランキングデータの正確性検証と修正 (#55) |
| `6716e5b4` | fix: 1978年Exile(US)のアーティスト情報を修正 |

### レビュー対応

- **Codex指摘**: 1978年の"Kiss You All Over"はアメリカのカントリーバンドExileの曲
- **対応**: アーティストIDを`a-exile-us`に変更し、名前を`Exile`に修正
